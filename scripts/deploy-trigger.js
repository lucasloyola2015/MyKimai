...
BEGIN
    IF NEW.end_time IS NOT NULL AND NEW.start_time IS NOT NULL THEN
        ...
        -- Si billable es false, el monto es siempre 0.
...
async function main() {
    console.log("Desplegando trigger...");
    try {
        await prisma.$executeRawUnsafe(sql);
        console.log("Trigger desplegado correctamente.");

        // Sincronizar registros existentes que no son facturables
        console.log("Sincronizando registros no facturables...");
        await prisma.$executeRawUnsafe(`
            UPDATE public.time_entries 
            SET amount = 0, rate_applied = 0 
